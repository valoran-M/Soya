...
let tr_function (fdef : pseudo function_def) : pseudo function_def =
  ...
  (* Translate ---------------------------------------------------------------*)
    ...
    | None ->
      ...
      | IMove(rd,r,n)    -> push_node (IMove (rd,r, tr_instruction n))
      | IOp(op,rl,r,n)   -> push_node (IOp(op,rl,r,tr_instruction n))
      | ILoad(a,r,n)     -> push_node (ILoad (a,r, tr_instruction n))
      | IStore(a,r,n)    -> push_node (IStore(a,r, tr_instruction n))
      | ICall(id,lr,i,r,n)-> push_node (ICall(id,lr,i,r, tr_instruction n ))
      | ICond(c,lr,nt,nf) ->
